...
function init() {
  ...
  if (adminSession.role === 'normal') {
    ...
    // Initialize deadlines from config ONLY on first load ever
    if (!localStorage.getItem('courseDeadlinesInit')) {
        const globalDeadlines = (window.CourseGlobalConfig && window.CourseGlobalConfig.deadlines) || [];
        localStorage.setItem('courseDeadlines', JSON.stringify(globalDeadlines));
        localStorage.setItem('courseDeadlinesInit', 'true');
    }
...
function renderDeadlines() {
  ...
  deadlines.forEach((item, index) => {
    // Check if this item is already "Live" in config.js
    const isLive = globalDeadlines.some(g => g.name === item.name && g.date === item.date);
    
    const row = document.createElement('tr');
    if (isLive) row.style.background = '#f0f7ff';
    
    row.innerHTML = `
      <td>
        <strong>${item.name}</strong>
        ${isLive ? '<span class="admin-badge" style="font-size: 0.6rem; background: var(--dark-blue); margin-left: 5px;">Live</span>' : 
                   '<span class="admin-badge" style="font-size: 0.6rem; background: #6c757d; margin-left: 5px;">Draft</span>'}
      </td>
      <td>${new Date(item.date).toLocaleDateString()}</td>
      <td>
        ${isLive ? '<span style="color: #2e7d32; font-size: 0.8rem;"><i class="fas fa-check-circle"></i> Visible to Students</span>' : 
                   '<span style="color: #d32f2f; font-size: 0.8rem;"><i class="fas fa-clock"></i> Not synced yet</span>'}
      </td>
      <td>
  ...
  if (deadlines.length === 0) {
    tbody.innerHTML = '<tr><td colspan="4" style="text-align: center; padding: 20px; color: var(--gray-dark);">No deadlines set. Add your first one above!</td></tr>';
  }
}
...
function exportGlobalConfig() {
  ...
  const jsContent = `// Global Configuration for Web Dev Course
// This file is generated by the Admin Dashboard.
// To apply these changes: Upload this file to "modern-assets/js/config.js" in GitHub.

window.CourseGlobalConfig = {
    // These deadlines are visible to everyone
    deadlines: ${JSON.stringify(localDeadlines, null, 4)}
};`;
  ...
  modal.innerHTML = `
    <h3 style="margin-top: 0; color: var(--dark-blue);"><i class="fas fa-cloud-upload-alt"></i> Sync with Students</h3>
    <p style="margin-bottom: 15px; font-size: 0.95rem;">You have updated the deadlines. To make them visible to all students, you have two options:</p>
    
    <div style="background: #f8f9fa; padding: 15px; border-radius: 8px; margin-bottom: 20px;">
        <p style="font-weight: bold; margin-bottom: 10px; color: #1565c0;">Option 1 (Easiest - No Code):</p>
        <button id="downloadBtn" style="width: 100%; padding: 12px; background: #2196f3; color: white; border: none; border-radius: 6px; cursor: pointer; font-weight: bold; margin-bottom: 10px;">
            <i class="fas fa-file-download"></i> Download "config.js"
        </button>
        <p style="font-size: 0.8rem; color: #666;">Then simply upload/replace the file in <code>modern-assets/js/config.js</code> on GitHub.</p>
    </div>

    <div style="background: #fff3e0; padding: 15px; border-radius: 8px; margin-bottom: 20px;">
        <p style="font-weight: bold; margin-bottom: 10px; color: #e65100;">Option 2 (Copy Code):</p>
        <button id="copyBtn" style="width: 100%; padding: 10px; background: #ff9800; color: white; border: none; border-radius: 6px; cursor: pointer; font-weight: bold;">
            <i class="fas fa-copy"></i> Copy Config Code
        </button>
    </div>
